...
This document provides performance benchmarking of
[`geospatialsuite::quick_map()`](https://exelegch.github.io/geospatialsuite-docs/reference/quick_map.md)
against alternative R visualization approaches for large raster
datasets. The benchmarks demonstrate the package’s claims regarding
memory efficiency and robust error handling.

**Note**: These benchmarks require large external datasets (100+ MB) and
are not executed during package installation. The code is provided for
reproducibility and can be run with user-supplied data.
...
**Key Finding**: geospatialsuite and terra maintain constant ~75 MB
overhead regardless of raster size, while ggplot2 scales linearly with
pixels (22.9 → 572.2 MB). This represents a **7.6× memory advantage**
for realistic satellite imagery.
...
**Key Finding**: For small datasets, ggplot2’s optimized data frame
rendering is faster. However, for medium to large datasets (realistic
satellite imagery), geospatialsuite is 1.7-4.2× faster. The performance
advantage grows with data size due to the overhead of data frame
conversion.
...
**Critical Finding**: ggplot2 converts entire raster to data frame,
causing memory usage to scale linearly with raster size. For a 10K×10K
raster: - Terra/geospatialsuite: Maintains ~75 MB (constant, uses C++
rendering) - ggplot2: Requires ~8+ GB (linear scaling, R data frame
conversion)
...
leverages `terra`’s efficient raster handling and avoids converting
entire rasters to data frames (which ggplot2 requires). This results in:

- **Constant memory usage**: ~75 MB regardless of raster size (tested
  from 1M to 25M pixels)
- **No conversion overhead**: Works directly with terra SpatRaster
  objects using C++ rendering
- **Comparison**: ggplot2 scales from 22.9 MB (1K×1K) to 572 MB (5K×5K)
  due to data frame conversion - a **7.6× memory advantage** at
  realistic scales

**Why this matters**: For satellite imagery analysis, constant memory
usage means:

- Predictable resource requirements
- No memory-based failures
- Can process datasets that would require \>2 GB with ggplot2 approach
  (extrapolating to 10K×10K)

### 2. Performance at Scale

Performance characteristics vary by dataset size (based on actual
benchmark results):

**Small datasets (1K×1K, ~1M pixels)**: - ggplot2 faster (97ms vs 554ms)
due to optimized data frame rendering - However, ggplot2 uses 0.3× the
memory (22.9 MB vs 75 MB) - advantage to ggplot2 for very small data

**Medium datasets (3K×3K, ~9M pixels)**: - geospatialsuite becomes 1.7×
faster (600ms vs 1,025ms) - Memory advantage: 2.7× less memory (75 MB vs
206 MB)

**Large datasets (5K×5K, ~25M pixels)**: - geospatialsuite is 4.2×
faster (684ms vs 2,897ms) - Memory advantage: 7.6× less memory (75 MB vs
572 MB)

**Critical insight**: The C++ rendering backend maintains constant ~75
MB memory and speed increases only slightly with data size, while data
frame conversion overhead in ggplot2 causes both memory and time to
scale with pixels. The crossover point is around 2K×2K pixels (~4M
pixels).
...
Unlike methods that fail or become impractical at large scales,
[`quick_map()`](https://exelegch.github.io/geospatialsuite-docs/reference/quick_map.md)
...
|----|----|----|----|
| **geospatialsuite** | [`quick_map()`](https://exelegch.github.io/geospatialsuite-docs/reference/quick_map.md) | Auto-detection, constant memory (~75 MB), robust error handling, 4.2× faster at scale | Requires terra/sf |
| **terra** | [`plot()`](https://rspatial.github.io/terra/reference/plot.html) | Fast, efficient, constant memory (~75 MB) | Limited customization, basic error handling, manual data type handling |
| **ggplot2** | [`geom_raster()`](https://ggplot2.tidyverse.org/reference/geom_tile.html) | Beautiful outputs, flexible, fast for small data (\<2K×2K) | Data frame conversion (23→572 MB scaling), slow for large data |
...
- **geospatialsuite/terra**: Constant ~75 MB regardless of raster size
  (C++ backend)
- **ggplot2**: Linear scaling with pixels (22.9 MB → 206 MB → 572 MB for
  1K×1K → 3K×3K → 5K×5K)

**Speed at Scale** (actual benchmark results):

- **Small data (1K×1K)**: ggplot2 faster (97ms vs 554ms) - optimized for
  data frames
- **Medium data (3K×3K)**: geospatialsuite 1.7× faster (600ms vs
  1,025ms)
...
- **ggplot2**: Often fails or becomes impractical beyond 5K×5K due to
  memory requirements

### Unique Advantages of quick_map()

1.  **Universal data type support**: Works with SpatRaster, RasterLayer,
    sf objects, data frames
2.  **Constant memory usage**: ~75 MB regardless of data size
    vs. ggplot2 linear scaling (7.6× advantage at 5K×5K)
3.  **Built-in error recovery**: Gracefully handles common failure modes
    (missing data, CRS issues, empty rasters)
4.  **Performance at scale**: 4.2× faster than ggplot2 for realistic
    satellite imagery (5K×5K pixels)
5.  **Minimal dependencies**: Core functionality requires only terra and
    base R
6.  **Consistent API**: Same function call works for all spatial data
    types

### Performance Summary Table

**Actual benchmark results for realistic satellite imagery (5K×5K
pixels, 25M cells):**
...
**Bottom Line**:
[`quick_map()`](https://exelegch.github.io/geospatialsuite-docs/reference/quick_map.md)
combines terra’s memory efficiency with enhanced features and robust
error handling. For realistic satellite imagery analysis, it provides
7.6× better memory efficiency and 4.2× faster execution than data
...
1.  **Memory efficiency**: The key difference is that ggplot2 requires
    converting the entire raster to an R data frame, while
    terra/geospatialsuite use efficient C++ rendering. For a 5K×5K
    raster, the data frame alone requires ~4.5 GB vs terra’s constant
    ~75 MB rendering overhead.

2.  **Speed**: Performance advantage grows with dataset size - from
    comparable at small scales to 3-5× faster for realistic satellite
    imagery.

3.  **Reliability**: Robust error handling prevents common failures
    (missing data, CRS issues, empty rasters) that cause other
    approaches to crash.

4.  **Scalability**: Maintains consistent performance from small to very
    large datasets.

The function achieves these characteristics by: - Building on terra’s
...
Standard R memory profiling tools (including `bench::mark`’s memory
profiling) can conflict with terra’s internal parallelization. The
benchmarks above focus on:

- **Timing**: Measured reliably with `bench::mark(..., memory = FALSE)`
- **Memory footprint**: Measured via data structure sizes, which reveals
  the key difference (ggplot2’s data frame conversion vs direct
  rendering)

This approach accurately captures the performance characteristics
relevant to users: how fast the function runs and how much memory the
data transformations require.

These performance characteristics support the use of
[`quick_map()`](https://exelegch.github.io/geospatialsuite-docs/reference/quick_map.md)
in both interactive exploratory analysis and automated workflow
pipelines where reliability is critical.
